...
bazel_cmd=(
  ...
  "${passthrough_env[@]}"
  "${envs[@]}"
  "/Users/qhai-dev/Library/Caches/bazelisk/downloads/sha256/2db883718453f0437a7bcb408e889dbf8539cdc4d61c8ebc3807a1a88d02ff08/bin/bazel"
...
"$BAZEL_INTEGRATION_DIR/process_bazel_build_log.py" \
  ...
  "$output_groups_flag" \
  "@@rules_xcodeproj++rules_repos+rules_xcodeproj_generated//generator/frontend/apps/ios/xcodeproj:xcodeproj" \
...
if [[ -n "${target_ids:-}" ]]; then
  if [[ ! -s "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj++rules_repos+rules_xcodeproj_generated/generator/frontend/apps/ios/xcodeproj/xcodeproj_target_ids" ]]; then
      echo "error: \"$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj++rules_repos+rules_xcodeproj_generated/generator/frontend/apps/ios/xcodeproj/xcodeproj_target_ids\" was not created. This can happen if" \
...
"https://github.com/MobileNativeFoundation/rules_xcodeproj/issues/new?template=bug.md" \
  ...
  diff_output=$(
    comm -23 \
      <(printf '%s\n' "${target_ids[@]}" | sort) \
      <(sort "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj++rules_repos+rules_xcodeproj_generated/generator/frontend/apps/ios/xcodeproj/xcodeproj_target_ids")
  )
